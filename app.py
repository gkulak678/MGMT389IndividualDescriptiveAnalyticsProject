...
# -----------------------------
with tab_prompt:
    st.subheader("Prompt + Deployment Notes")
    st.write("This page documents the PDID framework, methodology, and final prompt engineered for the solution.")

    st.markdown("#### PDID Framework")
    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(
        "**Problem**  \nImprove visibility into CRM pipeline conversion, sales team performance, product performance, and account segment outcomes."
    )
    c2.markdown(
        "**Data**  \nUse sales opportunities, accounts, products, and sales team tables. Clean product names, typos, dates, and open/closed deal logic."
    )
    c3.markdown(
        "**Insights**  \nCalculate descriptive KPIs across teams, agents, products, quarters, and account segments to identify performance patterns."
    )
    c4.markdown(
        "**Deployment**  \nDeploy a Streamlit dashboard so a non-technical stakeholder can filter, compare, interpret, and export results."
    )
    ...
    st.markdown(
        ...
        - **Reason for approach:** The project goal is to explain historical CRM performance and identify patterns, not predict future deals.
        - **Core KPIs:** total opportunities, closed opportunities, win rate, won revenue, average won deal, revenue per closed opportunity, and average days to close.
        - **Quality indicators:** correct joins, consistent KPI definitions, proper exclusion of open opportunities from closed-deal metrics, and clear filters for stakeholder usability.
        - **Decision-maker value:** improves decision quality by making sales performance easier to compare across teams, products, quarters, and account segments.
        """
    )

    avg_days_prompt = f"{kpis['avg_days']:.1f}" if not pd.isna(kpis["avg_days"]) else "N/A"
    final_prompt = f"""
You are a commercial analytics assistant helping a VP of Sales, RevOps leader, or commercial finance stakeholder interpret a CRM sales opportunities dashboard.

Business context:
- The company sells B2B computer hardware.
- The dataset includes sales opportunities, accounts, products, and sales team assignments.
- The analytical approach is descriptive analytics, not predictive modeling.

Current filtered dashboard KPIs:
- Total opportunities: {whole(kpis['total_opps'])}
- Closed opportunities: {whole(kpis['closed_opps'])}
- Open opportunities: {whole(kpis['open_opps'])}
- Win rate: {pct(kpis['win_rate'])}
- Won revenue: {money(kpis['won_revenue'])}
- Average won deal: {money(kpis['avg_won_deal'])}
- Average days to close: {avg_days_prompt}

Instructions:
1. Summarize the selected pipeline view in clear executive language.
2. Identify the strongest and weakest areas across teams, agents, products, quarters, and account segments.
3. Explain whether the pattern appears related to volume, conversion, deal value, or sales-cycle efficiency.
4. Avoid overclaiming causality because the analysis is descriptive.
5. Recommend practical next steps for a sales or commercial finance decision maker.
6. Write the response so a non-technical stakeholder can understand it.
""".strip()

    st.text_area(
        "Final prompt engineered for executive interpretation",
        value=final_prompt,
        height=360,
    )
...
plotly""",
    ...
    st.markdown("#### Final submission reminder")
    st.markdown(
        """
        Submit the PDF slide deck, video recording, deployed app link, `app.py`, `requirements.txt`, dataset files, and final prompt engineered for the solution.
        """
    )
